...
void _showDomainAccessDeniedDialog(
  ...
  final DomainAccessDenied state,
) {
  unawaited(
    showDialog<void>(
      context: context,
      builder: (final dialogContext) => AlertDialog(
        // Temporary porkbun hardcode, as the installer will be refactored in the future
        title: Text('initializing.porkbun_domain_not_opted_in_title'.tr()),
        content: Text('initializing.porkbun_domain_not_opted_in'.tr()),
        actions: [
          TextButton(
            onPressed: () {
              Navigator.of(dialogContext).pop();
              unawaited(context.read<DomainSetupCubit>().load());
            },
            child: Text('basis.got_it'.tr()),
          ),
        ],
      ),
    ),
...
class _DomainPickerState extends State<DomainPicker> {
  ...
  Widget build(final BuildContext context) =>
      BlocConsumer<DomainSetupCubit, DomainSetupState>(
        listener: (final context, final state) {
          if (state is DomainAccessDenied) {
            _showDomainAccessDeniedDialog(context, state);
          }
        },
